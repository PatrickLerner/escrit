class ApplicationController < ActionController::Base
  ...
  def empty_html_response
    return render file: 'layouts/application' if request.format == :html
  end

  # always redirect away from the www-version of the site to the plain url one
  def redirect_subdomain
    www_redirect = request.host == 'www.escrit.eu'
    redirect_to "http://escrit.eu#{request.fullpath}" if www_redirect
  end
